$(document).ready(function () {
    ...
    $("#Save_MiscSuggestion_Btn").click(function (event) {

        if (validateAddMiscSuggestionMessage() == false) {
            //$("#btnSave").prop("disabled", true);
            return false;
        }

        // Get form
        var form = $('#AddformMiscSuggestion')[0];
        ...
        $.ajax({
            ...
            url: "/MiscSuggestion/AddMiscSuggestion",
...
var onSuccess = function (context) {
    $(".loaderModal").hide();
    swal(
        'The misc suggestion was successfully created.',
        '',
        'success'
    ).then(function () {
...
var validateAddMiscSuggestionMessage = function () {
    var isValid = true;
    debugger;

    // check if the form input is valid
    if (!$("#AddformMiscSuggestion").valid()) {
        $("#AddformMiscSuggestion").submit();
        isValid = false;
    }
