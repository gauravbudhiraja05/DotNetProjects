$(document).ready(function () {
    ...
    $("#Edit_ClassificationResult_Btn").click(function (event) {

        if (validateEditClassificationResult() == false) {
            //$("#btnSave").prop("disabled", true);
            return false;
        }

        // Get form
        var form = $('#EditformClassificationResult')[0];
        ...
        $.ajax({
            ...
            url: "/ClassificationResult/EditClassificationResult",
...
function GetDropDownValues() {
    var selectedTestId = $('#ClassificationId').val();
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.length > 0) {
                ...
                if (selectedTestId != undefined && parseInt(selectedTestId) > 0) {
                    $('#ddlClassification').val(selectedTestId);
                }
...
var onSuccess = function (context) {
    $(".loaderModal").hide();
    swal(
        'The classification result was successfully updated.',
        '',
        'success'
    ).then(function () {
...
var validateEditClassificationResult = function () {
    var isValid = true;
    debugger;

    // check if the form input is valid
    if (!$("#EditformClassificationResult").valid()) {
        $("#EditformClassificationResult").submit();
        isValid = false;
    }
