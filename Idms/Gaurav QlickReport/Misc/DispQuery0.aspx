...
--%>
<%@ Page Language="VB" AutoEventWireup="false" CodeFile="DispQuery0.aspx.vb" Inherits="Misc_DispQuery" %>
...
<head runat="server">
    <title>BFI</title>
		...
		{
			url = "DispQuery1.aspx?name=" + strname + "&dept=" + document.frmDispQuery.txtdept.value + "&client=" + document.frmDispQuery.txtclient.value + "&lob=" + document.frmDispQuery.txtlob.value + "&date=" + document.frmDispQuery.txtDate.value + "&date1=" + document.frmDispQuery.txtDate1.value
			window.open(url)
		}
		function winopen1(strname)
		{
			url = "DispQuery2.aspx?name=" + strname + "&dept=" + document.frmDispQuery.txtdept.value + "&client=" + document.frmDispQuery.txtclient.value + "&lob=" + document.frmDispQuery.txtlob.value + "&date=" + document.frmDispQuery.txtDate.value + "&date1=" + document.frmDispQuery.txtDate1.value
			window.open(url)
		}
...
<body>
    <form id="frmDispQuery" runat="server">
    	<input id=txtlob type="hidden" name=txtlob value="<%=Request("LOB")%>">
			...
			<input id=txtdept type="hidden" name=txtdept value="26" />
			...
			<table width="70%" >
				...
				<tr>
					...
					<br>
						...
						<input id="txtDate" style="FONT-SIZE: 12px; WIDTH: 140px; FONT-FAMILY: Verdana; HEIGHT: 18px"
							tabIndex="1" type="text" title="Enter Date" maxLength="10" size="28" name="txtDate" runat="server" onkeypress="if (event.keyCode >= 47 && event.keyCode <= 57){event.returnValue = true;} else {event.returnValue = false;} if(event.keyCode==8){charRemove();}" class="textBox">
							<input id="imageFromDate" style="BORDER-RIGHT: 0px solid; BACKGROUND: url(/AutoWhiz/Calendar/Calendar.gif); BORDER-LEFT: 0px solid; WIDTH: 16px; CURSOR: hand; BORDER-BOTTOM: 0px solid; BORDER-solid: "
							onclick="ShowCalendar(this.id);" tabIndex="1" type="button" name="imageFromDate" title="Select Date">
						&nbsp;&nbsp;&nbsp;&nbsp; <label for="txtDate1" class="label" title="End Date">End Date</label><input id="txtDate1" style="FONT-SIZE: 12px; WIDTH: 140px; FONT-FAMILY: Verdana; HEIGHT: 18px"
							tabIndex="1" type="text" maxLength="10" size="28" name="txtDate1" runat="server" onkeypress="if (event.keyCode >= 47 && event.keyCode <= 57){event.returnValue = true;} else {event.returnValue = false;} if(event.keyCode==8){charRemove();}" class="textBox"><input id="imageFromDate1" style="BORDER-RIGHT: 0px solid; BACKGROUND: url(/AutoWhiz/Calendar/Calendar.gif); BORDER-LEFT: 0px solid; WIDTH: 16px; CURSOR: hand; BORDER-BOTTOM: 0px solid; BORDER-solid: "
							onclick="ShowCalendar1(this.id);" tabIndex="2" type="button" name="imageFromDate"  title="Enter Date">
							
					</td>
				</tr>
				<tr>
					<td>
						<div id="divquery" runat="server" style="WIDTH: 100%; height: 71px;"></div>
					</td>
				</tr>
				<tr>
					<td style="overflow:auto;">
					<table width="100%"><caption>Saved HTML Files</caption></table>
						<iframe id="FRMRESULST" style="VISIBILITY: visible; WIDTH: 100%; HEIGHT: 350px; BACKGROUND-COLOR: white" src="ReportHTMLForm.aspx"
