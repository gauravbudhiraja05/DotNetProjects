        ...
        If rdr.Read Then
            ...
            If (producttype = "Multiple User") Then
                ...
                If (Session("typeofuser") = "Super Admin") Then
                    cmd2 = New SqlCommand("Select MenuDescription from nlvl_menu_FP as a, nlvl_menu as b where a.menuid=b.MenuID and a.createdby='" + Session("userid").ToString() + "'", connection)
                Else
                    cmd2 = New SqlCommand("Select MenuDescription from nlvl_menu_FP as a, nlvl_menu as b where a.menuid=b.MenuID and a.createdby='" + Session("CreatedBy").ToString() + "'", connection)
                End If
                Dim dsar As DataSet = New DataSet()
                Dim daar As SqlDataAdapter = New SqlDataAdapter(cmd2)
                daar.Fill(dsar)
...
#End Region
    ...
    Protected Sub ddlLobName_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlLobName.SelectedIndexChanged
        If (ddlLobName.SelectedValue = "" Or ddlLobName.SelectedValue = "--Select--") Then
            ddlReport.Items.Clear()
            ddlgraphname.Items.Clear()
            bind_report(Departmentname.SelectedValue, ClientName.SelectedValue, 0)
        Else
            deptid = Departmentname.SelectedValue
