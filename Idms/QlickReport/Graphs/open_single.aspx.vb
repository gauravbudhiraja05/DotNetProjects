...
Partial Class Graphs_open_single
    ...
    Protected Sub get_query_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles get_query.Click
        ...
        con.Open()
        cmd = New SqlCommand("Select Distinct(QueryName) from IdmsGraphMaster where DepartmentID='60' and ClientID='0' and UnderLOB='0' and  SavedBy='" + Session("userid") + "' ", con)
    ...
    Protected Sub queryname_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles queryname.SelectedIndexChanged
        ...
        con.Open()
        cmd = New SqlCommand("Select * from IdmsGraphMaster where DepartmentID='60' and ClientID='0' and UnderLOB='0' and SavedBy='" + Session("userid") + "' and QueryName='" + queryname.SelectedItem.Text + "'", con)
    ...
    Protected Sub ddlgraph_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlgraph.SelectedIndexChanged
        ...
        con.Open()
        cmd = New SqlCommand("Select * from IdmsGraphMaster where DepartmentID='60' and ClientID='0' and UnderLOB='0' and SavedBy='" + Session("userid") + "' and QueryName='" + queryname.SelectedItem.Text + "' and GraphName='" + ddlgraph.SelectedItem.Text + "' ", con)
        dr = cmd.ExecuteReader()
        While (dr.Read())
    ...
    Protected Sub plotgraph_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles plotgraph.Click
        ...
        If (gptype.Equals("Point")) Then
            chart_plot.Series(0).ChartType = Point
        End If
        If (gptype.Equals("FastPoint")) Then
            chart_plot.Series(0).ChartType = FastPoint
        End If
        If (gptype.Equals("Bubble")) Then
            chart_plot.Series(0).ChartType = Bubble
        End If
        If (gptype.Equals("Line")) Then
            chart_plot.Series(0).ChartType = Line
        End If
        If (gptype.Equals("Spline")) Then
            chart_plot.Series(0).ChartType = Spline
        End If
        If (gptype.Equals("StepLine")) Then
            chart_plot.Series(0).ChartType = StepLine
        End If
        If (gptype.Equals("FastLine")) Then
            chart_plot.Series(0).ChartType = FastLine
        End If
        If (gptype.Equals("Column")) Then
            chart_plot.Series(0).ChartType = Column
        End If
        If (gptype.Equals("StackedColumn")) Then
            chart_plot.Series(0).ChartType = StackedColumn
        End If
        If (gptype.Equals("StackedColumn100")) Then
            chart_plot.Series(0).ChartType = StackedColumn100
        End If
        If (gptype.Equals("Area")) Then
            chart_plot.Series(0).ChartType = Area
        End If
        If (gptype.Equals("SplineArea")) Then
            chart_plot.Series(0).ChartType = SplineArea
        End If
        If (gptype.Equals("StackedArea")) Then
            chart_plot.Series(0).ChartType = StackedArea
        End If
        If (gptype.Equals("StackedArea100")) Then
            chart_plot.Series(0).ChartType = StackedArea100
        End If
        If (gptype.Equals("Stock")) Then
            chart_plot.Series(0).ChartType = Stock
        End If
        If (gptype.Equals("Candlestick")) Then
            chart_plot.Series(0).ChartType = Candlestick
        End If
        If (gptype.Equals("Range")) Then
            chart_plot.Series(0).ChartType = Range
        End If
        If (gptype.Equals("SplineRange")) Then
            chart_plot.Series(0).ChartType = SplineRange
        End If
        If (gptype.Equals("ErrorBar")) Then
            chart_plot.Series(0).ChartType = ErrorBar
        End If
        If (gptype.Equals("BoxPlot")) Then
            chart_plot.Series(0).ChartType = BoxPlot
        End If
        If (i.Equals(3)) Then
            chart_plot.Series(0).XValueMember = ds1.Tables(0).Columns(s1).ToString()
            chart_plot.Series(0).YValueMembers = ds1.Tables(0).Columns(s2).ToString()
            chart_plot.Series(1).YValueMembers = ds1.Tables(0).Columns(s3).ToString()
        ElseIf (i.Equals(2)) Then
            chart_plot.Series(0).XValueMember = ds1.Tables(0).Columns(s1).ToString()
            chart_plot.Series(0).YValueMembers = ds1.Tables(0).Columns(s2).ToString()
        End If
    ...
    Protected Sub update_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles update.Click
        ...
        For k = 0 To a2 - 1
            ...
            End If
        Next
        cmd = New SqlCommand("Update IdmsGraphMaster set GraphType='" + gtype + "',DepartmentID='60',ClientID='0',UnderLOB='0',QueryName='" + queryname.SelectedItem.Text + "',ColumnName='" + sltcol + "',SavedBy='" + Session("userid") + "',Totalcolumn='" + repcol + "'where GraphName='" + ddlgraph.SelectedItem.Text + "'", con)
        con.Open()
        dr = cmd.ExecuteReader()
    ...
    Protected Sub update_axis_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles update_axis.Click
        ...
        '    chart_plot.Series(0).XValueMember = ds1.Tables(0).Columns(x1).ToString()
        '    chart_plot.Series(0).YValueMembers = ds1.Tables(0).Columns(y1).ToString()
        '    chart_plot.Series(1).YValueMembers = ds1.Tables(0).Columns(z1).ToString()
    ...
    Protected Sub plt_upgrph_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles plt_upgrph.Click
        ...
        Dim i = selectedcols.Items.Count
        If (i.Equals(2)) Then
            x = ddl_xaxis.SelectedItem.Text
            y = ddl_yaxis1.SelectedItem.Text
        ElseIf (i.Equals(3)) Then
            x = ddl_xaxis.SelectedItem.Text
            y = ddl_yaxis1.SelectedItem.Text
            z = ddl_yaxis2.SelectedItem.Text
        End If
        Dim s As String = queryname.SelectedItem.Text
        ...
        da1.Fill(ds1)
        If (i.Equals(2)) Then
            chart_plot.Series(0).XValueMember = ds1.Tables(0).Columns(x).ToString()
            chart_plot.Series(0).YValueMembers = ds1.Tables(0).Columns(y).ToString()
        ElseIf (i.Equals(3)) Then
            chart_plot.Series(0).XValueMember = ds1.Tables(0).Columns(x).ToString()
            chart_plot.Series(0).YValueMembers = ds1.Tables(0).Columns(y).ToString()
            chart_plot.Series(1).YValueMembers = ds1.Tables(0).Columns(z).ToString()
        End If
        chart_plot.DataSource = ds1
        chart_plot.DataBind()
        Dim a1, a2, k As Integer
