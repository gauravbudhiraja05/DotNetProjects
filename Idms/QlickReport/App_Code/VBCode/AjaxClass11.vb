...
Public Class AjaxClass11
    ...
    <Ajax.AjaxMethod()> Public Function bindtabs(ByVal lobid As String) As String
        ...
        Return str
    End Function
    <Ajax.AjaxMethod()> Public Function bindtable(ByVal deptid As String, ByVal clientid As String, ByVal lobid As String) As String
        Dim cmdgettab As New SqlCommand
        If Session("usertype") = "member" Then
            cmdgettab = New SqlCommand("select LTrim(RTrim(TableName)) as TableName,Visiblecolumn from warslobtablemaster where  LOBid='" & lobid & "' and DepartmentId='" & deptid & "' and ClientId='" & clientid & "' and tablename in (select tablename from warslobtablerights where userid='" & Session("userid") & "') and Editable='Yes' and Importable='Yes' order by TableName", connection)
        Else
