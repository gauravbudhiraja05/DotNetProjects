...
Partial Class QueryBuilder_SavedQueries
        ...
        If Me.IsPostBack = False Then
            ...
            If rdr.Read Then
                ...
                If (producttype = "Multiple User") Then
                    ...
                    lob_id.Visible = True
                    multi_ddl.Visible = True
                    Button1.Visible = True
                    ddldept_bind()
                Else
                    button_id.Visible = True
                    single_ddl.Visible = True
                    Button1_single.Visible = True
                End If
            End If
            connection.Close()
            rdr.Close()
    ...
    Public Function ddldept_bind()
        ...
        Try
            Dim comdepart As New SqlCommand("Select DepartmentName,Autoid  from idmsdepartment where savedBy='" + Session("userid") + "'", connection)
    ...
    Protected Sub ddlQueryName_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlQueryName.SelectedIndexChanged
        Try
            ...
            cmd.CommandText = "select * from warsquerymaster where DepartmentId='" & cbodept.SelectedValue & "' and ClientId='" & client & "' and lobyname='" & lob & "' and queryname = '" & ddlQueryName.SelectedItem.Text & "'"
            con.Open()
            ...
            con.Close()
            cmd.Dispose()
            Session("mailtain") = Session("mailtain") + "," + cbodept.SelectedValue.ToString + "," + client.ToString + "," + lob.ToString + "," + ddlQueryName.SelectedItem.Text
    ...
    End Function
    Public Function ddlQueryName_bind_single()
        Try
            '''''''''''''''''''''''filling of table dropdownlist'''''''''''''''''''''''''
            'Session("username")
            Dim selectRep As New ReportDesigner
            ...
            Dim cmduser As New SqlCommand("select queryName,sharedwith from warsquerymaster where DepartmentId='60' and ClientId='0' and lobyname='0' and sharedwith is not null order by queryName", connection)
            'Dim cmduser As New SqlCommand(qstr, connection)
            ...
            Do While rdusern.Read
                ...
                Dim j As Integer
                ddlQueryName.DataTextField = "queryName"
                ddlQueryName.DataValueField = "queryName"
                ddlQueryName.DataBind()
                sharewith = rdusern("sharedwith")
                Dim str As String()
                str = Split(Trim(sharewith), ",")
    ...
    Protected Sub ddlYourQuery_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlYourQuery.SelectedIndexChanged
        Try
            ...
            If Me.ddlYourQuery.SelectedIndex > 0 Then
                ...
                'Response.End()
                cmd.CommandText = "select * from warsquerymaster where DepartmentId='" & cbodept.SelectedValue & "' and ClientId='" & client & "' and lobyname='" & lob & "' and queryname = '" & ddlYourQuery.SelectedItem.Text & "' order by queryName"
                con.Open()
                dr = cmd.ExecuteReader
                While dr.Read = True
                    chkPercentage.Value = dr("Percentage").ToString
                ...
                con.Close()
                cmd.Dispose()
                Session("mailtain") = Session("mailtain") + "," + cbodept.SelectedValue.ToString + "," + client.ToString + "," + lob.ToString + "," + ddlYourQuery.SelectedItem.Text
            Else
                cmddelete.Visible = True
            End If
    ...
    Protected Sub cmddelete_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmddelete.Click
        Try
            If Me.ddlYourQuery.SelectedIndex <= 0 And ddlQueryName.SelectedIndex <= 0 Then
            ...
            If Me.ddlYourQuery.SelectedIndex <= 0 Then
                q_name = ddlQueryName.SelectedItem.Text
            Else
            ...
            If Session("usertype") = "superadmin" Then
                ...
                'Dim str1 = objQB.trackQ_BuilderForMaster(Session("userid"), "Delete", "Query", ddlYourQuery.SelectedItem.Text, "  ", dept, client, lob)
            ...
            Else
                ...
                '''''''''''''''Usertype check for track goes here:- By Suvidha
                Dim objQB As New QueryBuilder()
                'Dim str1 = objQB.trackQ_BuilderForMaster(Session("userid"), "Delete", "Query", q_name, "", dept, client, lob)
    ...
    Protected Sub cbolob_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cbolob.SelectedIndexChanged
        ...
        ddlYourQuery_bind()
    End Sub
    Public Function getheading(ByVal d, ByVal client, ByVal lob, ByVal hidtablename) As String
        Dim result As String
        If ddlQueryName.SelectedIndex <> 0 Then
            Dim cmdsave As New SqlCommand("select queryName from warsquerymaster where DepartmentId='" & d & "' and ClientId='" & client & "' and lobyname='" & lob & "' and tableName='" & hidtablename & "'", connectionsave)
        ...
        Else
            Dim cmdsave3 As New SqlCommand("select queryName from warsquerymaster where DepartmentId='" & d & "' and ClientId='" & client & "' and lobyname='" & lob & "' and tableName='" & hidtablename & "'", connectionsave2)
    ...
    Public Function getsub()
        ...
        result1 = getheading(dept, client, lob, hidtablename.Value)
        Return result1
    ...
    Public Function ddlYourQuery_bind()
        ...
        Try
            ...
            If (Session("typeofuser") = "Admin") Then
                ...
                If readerdata.HasRows Then
                    qstr = "select queryName from warsquerymaster where DepartmentId='" & cbodept.SelectedValue & "' and ClientId='" & client & "' and lobyname='" & lob & "' order by queryName"
                Else
                    qstr = "select queryName from warsquerymaster where DepartmentId='" & cbodept.SelectedValue & "' and ClientId='" & client & "' and lobyname='" & lob & "' and savedby='" + Session("userid") + "' order by queryName"
                ...
                con.Close()
            Else
                'Dim scope = Trim(selectRep.chkUserscope(Session("userid"), cbodept.SelectedValue, client, lob))
                'If (scope = "Local") Then
                'qstr = "select queryName from warsquerymaster where DepartmentId='" & cbodept.SelectedValue & "' and ClientId='" & client & "' and lobyname='" & lob & "' and (status='Local' or savedby='" + Session("userid") + "') order by queryName"
        ...
        End Try
    End Function
    Public Function ddlYourQuery_bind_single()
        '''''''''''''''''''''''filling of table dropdownlist'''''''''''''''''''''''''
        Try
            Dim selectRep As New ReportDesigner
            Dim dept
            ...
            If (Session("typeofuser") = "Admin") Then
                ...
                If readerdata.HasRows Then
                    qstr = "select queryName from warsquerymaster where DepartmentId='" & dept & "' and ClientId='" & client & "' and lobyname='" & lob & "' order by queryName"
                Else
                    qstr = "select queryName from warsquerymaster where DepartmentId='" & dept & "' and ClientId='" & client & "' and lobyname='" & lob & "' and savedby='" + Session("userid") + "' order by queryName"
            ...
            adp.Fill(ds, "warsquerymaster")
            connection.Close()
            ddlYourQuery_single.DataSource = ds
            ddlYourQuery_single.DataTextField = "queryName"
            ddlYourQuery_single.DataBind()
            ddlYourQuery_single.Items.Insert(0, "--select--")
            objCbo1Cmd.Dispose()
        Catch ex As Exception
            Dim strmsg As String
            strmsg = Replace(ex.Message.ToString, "'", "")
    ...
    Protected Sub cmddelete_single_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmddelete_single.Click
        Try
            If Me.ddlYourQuery_single.SelectedIndex <= 0 And ddlQueryName.SelectedIndex <= 0 Then
                ShowConfirm("Please select any one from your query.")
                Exit Sub
            End If
            Dim dept As String
            ...
            Dim q_name As String = ""
            'If Me.ddlYourQuery.SelectedIndex <= 0 Then
            q_name = ddlYourQuery_single.SelectedItem.Text
            'Else
            '    q_name = ddlYourQuery.SelectedItem.Text
            'End If
            'If Session("usertype") = "2" And ddlYourQuery_single.SelectedIndex > 0 Then
            'Else
            ...
            'If Me.ddlYourQuery.SelectedIndex > 0 Then
            'If Session("usertype") = "superadmin" Then
            '    Dim cmdins As New SqlCommand("insert into warsquerydeletelog select tableName,wheredata,wheredata1,showData,crdata,colName,selected_field,formula,queryName,savedBy,createDate,lobyName,sharedwith,Percentage,FormulaName,DepartmentId,ClientId,removesharing,getdate() from warsquerymaster where queryname='" & ddlYourQuery_single.SelectedItem.Text & "'", con)
            Dim cmddelete As New SqlCommand("delete warsquerymaster where queryname like '" & ddlYourQuery_single.SelectedItem.Text & "'", con)
            Dim cmddelete1 As New SqlCommand("delete subtotal where queryName= '" & ddlYourQuery_single.SelectedItem.Text & "'", con)
            con.Open()
            'cmdins.ExecuteNonQuery()
            cmddelete.ExecuteNonQuery()
            cmddelete1.ExecuteNonQuery()
            ...
            Dim cmm As New SqlCommand("insert into Querybuilder_utype select MAX(Auto_id)," + Session("usertype") + " from Track_QueryBuilder where Query_Name='" + ddlYourQuery_single.SelectedItem.Text + "' and Action='Delete'", connection)
            connection.Open()
            cmm.ExecuteNonQuery()
            connection.Close()
            '''''''''''''''Usertype check for track goes here:- By Suvidha
            'Else
            ''''''''No rights chk
            'Dim cmddelete11 As New SqlCommand("delete warsquerymaster where queryname = '" & q_name & "' and departmentid='" + dept + "' and clientid='" + client + "' and lobyname='" + lob + "'", con)
            'con.Open()
            'cmddelete.ExecuteNonQuery()
            'Dim cmddelete1 As New SqlCommand("delete subtotal where queryName= '" & q_name & "'", con1)
            'con1.Open()
            ...
            ddlYourQuery_bind_single()
            'ddlQueryName_bind_single()
            ShowConfirm("Your query has been deleted successfully")
            'End If
        ...
        Catch ex As Exception
            ...
            ShowConfirm(strmsg)
        End Try
        ddlYourQuery_single.SelectedIndex = 0
    End Sub

    Protected Sub get_query_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles get_query.Click
        ddlYourQuery_bind_single()
    End Sub

    Protected Sub ddlYourQuery_single_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlYourQuery_single.SelectedIndexChanged
        Try
            'If Me.ddlQueryName.SelectedIndex > 0 Then
            '    ShowConfirm("Please select any one from your query and shared query ")
            '    Me.ddlYourQuery.SelectedIndex = 0
            ...
            'End If
            'ddlYourQuery
            If Me.ddlYourQuery_single.SelectedIndex > 0 Then
                cmddelete_single.Visible = True
                Dim dept
                Dim client
                Dim lob
                ...
                'Response.Write("select * from warsquerymaster where queryname like '" & ddlYourQuery.SelectedItem.Text & "' and ',' + sharedwith + ',' like '%," & Session("userid") & ",%' ")
                'Response.End()
                cmd.CommandText = "select * from warsquerymaster where DepartmentId='" & dept & "' and ClientId='" & client & "' and lobyname='" & lob & "' and queryname = '" & ddlYourQuery_single.SelectedItem.Text & "' order by queryName"
                con.Open()
                dr = cmd.ExecuteReader
                While dr.Read = True
                    chkPercentage.Value = dr("Percentage").ToString
                ...
                con.Close()
                cmd.Dispose()
                Session("mailtain") = Session("mailtain") + "," + dept.ToString() + "," + client.ToString + "," + lob.ToString + "," + ddlYourQuery_single.SelectedItem.Text
            Else
                cmddelete_single.Visible = True
            End If
