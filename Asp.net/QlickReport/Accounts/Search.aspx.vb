    ...
    Private Sub WARSSearchMemReg()
        ...
        If txtSearch.Text <> "" Then
            ...
            ElseIf cboSearchCriteria.SelectedValue = "Resigned" Then
                Qstr = "select * from registration where lockreason ='$Resigned' and CreatedBy='" + Session("userid") + "' order by   "
            ElseIf cboSearchCriteria.SelectedValue = "Transfered" Then
                Qstr = "select * from registration where lockreason = '$Transfer' and CreatedBy='" + Session("userid") + "' order by "
            ...
            ElseIf cboSearchCriteria.SelectedValue <> "Search All" And cboSearchCriteria.SelectedValue <> "DepartmentId" And cboSearchCriteria.SelectedValue <> "LOBDept" And cboSearchCriteria.SelectedValue <> "ClientId" Then
                Qstr = "select A.recid,A.usertype,A.userid,A.prefix,A.username,A.designation,A.bu,A.email,A.status,A.empid,A.pwdstatus,isnull(A.lockreason,'') as lockreason,A.lockdate,A.createdby,A.localuser,A.deptid,A.clientid,A.lobid,A.creatorid,A.adminid, convert(varchar,A.adddate,111) as adddate from registration A where  A.usertype<>3 and " & Trim(Me.cboSearchCriteria.SelectedValue) & " like '" & Trim(Me.txtSearch.Text) & "%' and CreatedBy='" + Session("userid") + "' order by "
        ...
        ElseIf txtSearch.Text = "" Then
            If cboSearchCriteria.SelectedValue = "Search All" Then
                Qstr = "select recid,usertype,userid,prefix,username,designation,bu,email,status,empid,pwdstatus,isnull(lockreason,'') as lockreason,lockdate,createdby,localuser,deptid,clientid,lobid,creatorid,adminid, convert(varchar,adddate,111) as adddate from registration where usertype<>3 and CreatedBy='" + Session("userid") + "' order by "
            ElseIf cboSearchCriteria.SelectedValue = "Resigned" Then
                Qstr = "select * from registration where lockreason ='$Resigned' and CreatedBy='" + Session("userid") + "' order by   "
            ElseIf cboSearchCriteria.SelectedValue = "Transfered" Then
                Qstr = "select * from registration where lockreason = '$Transfer' and CreatedBy='" + Session("userid") + "' order by  "
